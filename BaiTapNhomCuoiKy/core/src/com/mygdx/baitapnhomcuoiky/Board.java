...
public class Board {
    ...
    //Auxiliary board classes
    private MultipleMatch _matches = new MultipleMatch();
    //Six matches possible per column and per row
    ...
    public void swap(int x1, int y1, int x2, int y2) {
        ...
        //Swapping two nearby gems with the condition:
        //		if (x1 >= 0 && x1 < 8 &&
        //			y1 >= 0 && y1 < 8 &&
        //			x2 >= 0 && x2 < 8 &&
        //			y2 >= 0 && y2 < 8)
    ...
    public void generate() {
        ...
        //Generate the gem board when staring the game
    ...
    public Array<Coord> solutions() {
        //This is what happens when you swap two gems to make a match-3
        _results.clear();
        int currCoord = 0;
    ...
    public MultipleMatch check() {
        ...
        for (int y = 0; y < 8; ++y) {
            ...
            for (int x = 0; x < 6; ++x) {
                ...
                //Adding a match to score points
                for (k = x + 1; k < 8; ++k) {
                ...
                if (currentRow.size > 2) _matches.add(currentRow);

                x = k - 1;
            }
        }
        // Then, we check each column (vertical)
        for (int x = 0; x < 8; ++x) {
            //Checking for matches in a column
            for (int y = 0; y < 6; ++y) {
    ...
    public void fillSpaces() {
        //When the three matching gems are cleared, a new gem above will
        //fall into three cleared squares
        for(int x = 0; x < 8; ++x){
